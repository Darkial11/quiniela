...
def inicio(request, torneo_slug, jornada=1):
    ...
    jornada_obj = Jornada.objects.filter(torneo=torneo_obj, numero=jornada).first()
    if not jornada_obj:
        return HttpResponse(status=404)
    partidos = Partido.objects.filter(jornada=jornada_obj).order_by("fecha_partido")
    jornadas_ids = (
        Partido.objects.filter(jornada__torneo=torneo_obj)
        .values_list("jornada", flat=True)
        .distinct()
    )
    jornadas = Jornada.objects.filter(id__in=jornadas_ids).order_by("numero")
    if torneo_obj.tipo_cobro == "unico":
        ...
        pago_confirmado_jornada = Pago.objects.filter(
            user=request.user, jornada=jornada_obj, confirmado=True
        ).exists()
    return render(
        request,
...
def guardar_pronosticos(request, torneo_slug):
    if request.method == "POST":
        ...
        primer_partido = Partido.objects.get(id=pronosticos[0]["partido_id"])
        ...
        for item in pronosticos:
            ...
            Pronostico.objects.create(
                user=user, partido=partido, seleccion=item["seleccion"]
            )
...
def ver_pronosticos(request, torneo_slug):
    ...
    if not torneo_obj:
        return HttpResponse(status=404)
    todas_jornadas = Jornada.objects.filter(torneo=torneo_obj).order_by("numero")
    ...
    partidos = list(Partido.objects.filter(jornada=jornada).order_by("id"))
    if torneo_obj.tipo_cobro == "unico":
        participantes = list(
            User.objects.filter(perfil__pago_confirmado=True)
            .distinct()
            .select_related("perfil")
        )
    else:
        participantes = list(
            User.objects.filter(pago__jornada=jornada, pago__confirmado=True)
            .distinct()
            .select_related("perfil")
        )
    ...
    for participante in participantes:
        ...
        for partido in partidos:
            ...
            if pronostico:
                ...
                seleccion = "-"
                acierto = False
            fila_pronosticos.append({"seleccion": seleccion, "acierto": acierto})
        nick = getattr(
            getattr(participante, "perfil", None), "nick", participante.username
        )
        tabla.append({"nombre": nick, "pronosticos": fila_pronosticos, "total": total})
    ...
    bloque = {"jornada": jornada, "partidos": partidos, "tabla": tabla}
    return render(
        request,
        "quiniela/pronosticos.html",
        {
...
def cargar_pronosticos(request, torneo_slug, jornada):
    ...
    for pronostico in pronosticos:
        data.append(
            {"partido_id": pronostico.partido.id, "seleccion": pronostico.seleccion}
        )
...
def exportar_pdf_jornada(request, torneo_slug, jornada):
    ...
    if torneo_obj.tipo_cobro == "unico":
        participantes = sorted(
            ...
            key=lambda u: getattr(
                getattr(u, "perfil", None), "nick", u.username
            ).lower(),
        )
    else:
        participantes = sorted(
            User.objects.filter(
                pago__jornada=jornada_obj, pago__confirmado=True
            ).distinct(),
            key=lambda u: getattr(
                getattr(u, "perfil", None), "nick", u.username
            ).lower(),
    ...
    response["Content-Disposition"] = f'attachment; filename="jornada_{jornada}.pdf"'
    ...
    titulo = Paragraph(
        f"<b>{torneo_obj.nombre} - Jornada {jornada}</b>", estilos["Title"]
    )
    ...
    for participante in participantes:
        ...
        for partido in partidos:
            pronostico = Pronostico.objects.filter(
                user=participante, partido=partido
            ).first()
            seleccion = pronostico.seleccion if pronostico else "-"
            fila.append(seleccion)
        data.append(fila)
        posicion += 1
    ancho_pagina_util = landscape(letter)[0] - 40
    ancho_posicion = 28
    ancho_participante = 85
    ancho_restante = ancho_pagina_util - ancho_posicion - ancho_participante
    if partidos:
        ancho_partido = ancho_restante / len(partidos)
    else:
        ancho_partido = 60
    anchos_columnas = [ancho_posicion, ancho_participante]
    anchos_columnas += [ancho_partido] * len(partidos)
    ...
    doc.build(
        elementos, onFirstPage=agregar_numero_pagina, onLaterPages=agregar_numero_pagina
    )
    return response
...
def reglamento_torneo(request, torneo_slug):
    ...
    try:
        return render(request, template_name, {"torneo": torneo_obj})
    except TemplateDoesNotExist:
        return render(
            request, "quiniela/reglamento_generico.html", {"torneo": torneo_obj}
        )
...
def admin_cargar_pronostico(request):
    ...
    if jornada_id:
        jornada_obj = (
            Jornada.objects.select_related("torneo").filter(id=jornada_id).first()
        )
    if user_id:
        usuario_obj = User.objects.select_related("perfil").filter(id=user_id).first()
    if jornada_obj:
    ...
    if jornada_obj and usuario_obj:
        existentes = Pronostico.objects.filter(
            user=usuario_obj, partido__jornada=jornada_obj
        )
        pronosticos_existentes = {p.partido_id: p.seleccion for p in existentes}
        ya_existian = existentes.exists()
        if jornada_obj.torneo.tipo_cobro == "por_jornada":
            pago_obj = Pago.objects.filter(
                user=usuario_obj, jornada=jornada_obj
            ).first()
        ...
        if ya_existian and not confirmar_sobrescritura:
            ...
            for partido in partidos:
                ...
                if seleccion in ("L", "E", "V"):
                    Pronostico.objects.create(
                        user=usuario_obj, partido=partido, seleccion=seleccion
                    )
                    guardados += 1
