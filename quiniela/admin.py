...
from .models import Partido, Pronostico, Jornada, Torneo, Pago
...
def confirmar_pago(modeladmin, request, queryset):
    actualizados = queryset.filter(confirmado=False).update(
        confirmado=True, fecha_confirmacion=timezone.now(), confirmado_por=request.user
    )
    modeladmin.message_user(
        request, f"{actualizados} pago(s) confirmado(s).", messages.SUCCESS
    )
